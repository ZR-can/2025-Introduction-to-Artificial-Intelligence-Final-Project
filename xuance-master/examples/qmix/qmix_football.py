...
def parse_args():  
    parser = argparse.ArgumentParser("Example: QMIX of XuanCe for Google Football Research environments.")
    ...
    parser.add_argument("--config", type=str, default=f"./qmix_football_configs/{parser.parse_args().env_id}.yaml")
...
class Runner():
    ...
    def get_actions(self, obs_n, avail_actions, rnn_hidden, test_mode=False):
        rnn_hidden_next, actions_n = self.agents.act(obs_n, *rnn_hidden, avail_actions=avail_actions,
                                                     test_mode=test_mode)
