import os  # 操作系统相关功能，如文件路径、环境变量等
import socket  # 网络通信，获取主机名等网络操作
import time  # 时间相关操作，如延时、计时等
from pathlib import Path  # 面向对象的文件系统路径操作
import wandb  # Weights & Biases库，用于实验跟踪和可视化
import argparse  # 命令行参数解析，通过命令行参数自由定制程序行为
from copy import deepcopy  # 深拷贝数据结构
import numpy as np  # 数值计算库，提供数组和矩阵运算
from torch.utils.tensorboard import SummaryWriter  # PyTorch的TensorBoard日志记录工具
from xuance import get_arguments  # 从xuance包导入获取参数的方法
from xuance.environment import make_envs  # 从xuance.environment导入创建环境的函数
from xuance.torch.utils.operations import set_seed  # 从xuance.torch导入设置随机种子的函数
...
def parse_args():
    ...
    parser.add_argument("--test_mode", type=int, default=0)#test_mode   #1
    parser.add_argument("--device", type=str, default="cuda:0")
    parser.add_argument("--benchmark", type=int, default=1)#0
...
class Runner(object):
    ...
    def log_infos(self, info: dict, x_index: int):
        ...
        """
        if x_index <= self.running_steps:   #检查当前步数是否未超过预设的总步数
            if self.use_wandb:
                ...
                for k, v in info.items():
                    try:
                        self.writer.add_scalar(k, v, x_index)# 记录单值，记录单个数值
                    except:
                        self.writer.add_scalars(k, v, x_index) # 记录多值，同时记录一组相关数值
