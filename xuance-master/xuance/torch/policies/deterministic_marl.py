...
class TDDPG_policy(Basic_DDPG_policy):
    def __init__(self,
                 action_space: spaces_pettingzoo,
                #  observation_space: spaces_pettingzoo,
                 n_agents: int,
                 representation: nn.Module,
                 actor_hidden_size: Sequence[int],
                 critic_hidden_size: Sequence[int],
        ...
        super(Basic_DDPG_policy, self).__init__()
        self.action_dim = action_space.shape[0]
        # self.observation_dim = observation_space.state.p_pos
        # self.adver_pos = observation_space.other.adversary.p_pos
        # self.agents_pos = observation_space.other.agent.p_pos
        # self.landmark_pos = observation_space.landmark.p_pos
        ...
        self.critic_net = CriticNet(True, representation.output_shapes['state'][0], n_agents, self.action_dim,
                                critic_hidden_size, normalize, initialize, activation, device)
        # self.critic1_net = CriticNet(True, representation.output_shapes['state'][0], n_agents, self.action_dim,
        #                             critic_hidden_size, normalize, initialize, activation, device)
        # self.critic2_net = CriticNet(True, representation.output_shapes['state'][0], n_agents, self.action_dim,
        #                             critic_hidden_size, normalize, initialize, activation, device)
        self.target_actor_net = copy.deepcopy(self.actor_net)
        self.target_critic_net = copy.deepcopy(self.critic_net)
        self.parameters_actor = list(self.representation.parameters()) + list(self.actor_net.parameters())
        self.parameters_critic = self.critic_net.parameters()
        # self.parameters_critic2 = self.critic2_net.parameters()
        # self.parameters_critic = self.parameters_critic1*0.8+self.parameters_critic2*0.2
        #self.parameters_critic = [item*0.8 for item in self.parameters_critic1] + [item*0.2 for item in self.parameters_critic2]


    # def Pgddpg(self,):
    #     uatt = torch.pow(torch.abs(self.agents_pos,self.adver_pos),2)
    #     d = torch.sqrt(torch.sub(self.adver_pos,self.))
    ...
    def critic(self, observation: torch.Tensor, actions: torch.Tensor, agent_ids: torch.Tensor):
        outputs = self.representation(observation)
        critic_in = torch.concat([outputs['state'], actions, agent_ids], dim=-1)
        #self.critic_net = self.critic1_net(critic_in) ,self.critic2_net(critic_in)
        return self.critic_net(critic_in)
        # return self.critic1_net(critic_in)
    
    def critic2(self, observation: torch.Tensor, actions: torch.Tensor, agent_ids: torch.Tensor):
        outputs = self.representation(observation)
        critic_in = torch.concat([outputs['state'], actions, agent_ids], dim=-1)
        #self.critic_net = self.critic1_net(critic_in) ,self.critic2_net(critic_in)
        #return self.critic_net(critic_in)
        return self.critic2_net(critic_in)
    ...
    def soft_update(self, tau=0.005):
        ...
        for ep, tp in zip(self.critic_net.parameters(), self.target_critic_net.parameters()):
            tp.data.mul_(1 - tau)
            tp.data.add_(tau * ep.data)
        # for ep, tp in zip(self.critic2_net.parameters(), self.target_critic_net.parameters()):
        #     tp.data.mul_(1 - tau)
        #     tp.data.add_(tau * ep.data)
