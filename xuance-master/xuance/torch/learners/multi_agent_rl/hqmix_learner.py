...
class H_QMIX_Learner(LearnerMAS):
    def __init__(self, 
                 config, 
                 policy, 
                 optimizer, 
        ...
        super(H_QMIX_Learner).__init__(config, policy, optimizer, scheduler, device, model_dir)
    ...
    def update(self, sample):
        ...
        terminals = torch.Tensor(sample['terminals']).to(self.device)
        agent_mask = torch.Tensor(sample['agent_mask']).to(self.device)
    ...
    def update_recurrent(self, sample):
        ...
        _, q_next = self.policy.target_Q(obs.reshape(-1, episode_length + 1, self.dim_obs),
                             IDs.reshape(-1, episode_length + 1, self.dim_obs),
                             *target_rnn_hidden)
