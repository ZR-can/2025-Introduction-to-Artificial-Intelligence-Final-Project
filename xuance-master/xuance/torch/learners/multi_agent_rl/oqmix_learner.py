...
class OQMIX_Learner(LearnerMAS):
    def __init__(self,
                 config: Namespace,
                 policy: nn.Module,
        ...
        super(OQMIX_Learner, self).__init__(config, policy, optimizer, scheduler, device, model_dir)
    ...
    def update(self, sample):
        ...
        _, _, q_eval, entropy = self.policy(obs, IDs)
        print('当前的熵值是',entropy)
    ...
    def update_recurrent(self, sample):
        ...
        # print("每个智能体的q_eval_a值为::::gather时",q_eval_a)
        q_eval_a = q_eval_a.transpose(1, 2).reshape(-1, self.n_agents, 1)

        #add entropy to Q
        entropy = entropy[:, :(entropy.shape[1]-1)]
        entropy = entropy.reshape(-1, self.n_agents, 1)
        # print('entropy的形状是,',entropy.shape)
        q_eval_a += entropy
