...
class OQMIX_Agents(MARLAgents):
    ...
    def __init__(self,
        ...
        learner = OQMIX_Learner(config, policy, optimizer, scheduler,
                               config.device, config.model_dir, config.gamma,
                               config.sync_frequency)
        super(OQMIX_Agents, self).__init__(config, envs, policy, memory, learner, device,
    ...
    def act(self, obs_n, *rnn_hidden, avail_actions=None, test_mode=False):
        ...
        if self.use_recurrent:
            ...
            # print("batch_agents的长度是",avail_actions.shape)
            hidden_state, greedy_actions, _ , entropy= self.policy(obs_in.view(batch_agents, 1, -1),
                                                          agents_id.view(batch_agents, 1, -1),
                                                          *rnn_hidden,
                                                          avail_actions=avail_actions.reshape(batch_agents, 1, -1))
            greedy_actions = greedy_actions.view(batch_size, self.n_agents)
        else:
            hidden_state, greedy_actions, _ , entropy= self.policy(obs_in, agents_id, avail_actions=avail_actions)
        greedy_actions = greedy_actions.cpu().detach().numpy()
        # print("greedy_action为贪婪策略输出的动作值是",greedy_actions)
